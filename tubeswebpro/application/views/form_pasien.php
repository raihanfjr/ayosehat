<body>
    ...
    <div class="contact-clean">
        <form method="post" action="<?= base_url("c_pasien/jpasien/") . $res['id'] ?>" style="width: 900px;max-width: 900px;margin-top: 40px;">
            <h2 class="text-center">Isi Data Pasien</h2>
            <p>Nama*</p>
            <div class="form-group"><input class="form-control" type="text" name="name" id="name" placeholder="Nama Sesuai KTP Anda" value="<?= $this->session->userdata('name'); ?>" readonly></div>
            <p>Email*</p>
            ...
            <p>Tanggal Lahir*</p>
            <div class="form-group"><input class="form-control" type="date" name="dob" id="dob" value="<?= $this->session->userdata('birth_date'); ?>" readonly></div>
            <p>Jenis Kelamin*</p>
            <div class="form-group">
                <div class="form-check"><input class="form-check-input" type="radio" name="jenisk" id="jenisk" value="Pria" <?php if ($res['jenis_kelamin'] == 'Pria') {
                                                                                                                                echo ('checked="checked"');
                                                                                                                            } ?>><label class="form-check-label" for="formCheck-1">Pria</label></div>
                <div class="form-check"><input class="form-check-input" type="radio" name="jenisk" id="jenisk" value="Wanita" <?php if ($res['jenis_kelamin'] == 'Wanita') {
                                                                                                                                    echo ('checked="checked"');
                                                                                                                                } ?>><label class="form-check-label" for="formCheck-1">Wanita</label></div>
            </div>
            <?= form_error('jenisk', '<small class="text-danger pl-3">', '</small>'); ?>
            <p>Nama Dokter*</p>
            <div class="form-group"><input class="form-control" type="text" name="named" id="named" placeholder="Nama Dokter Yang Anda Pilih" value="<?= $res['name'] ?>" readonly></div>
            <p>Nama Rumah Sakit*</p>
            <div class="form-group"><input class="form-control" type="text" name="nameh" id="nameh" placeholder="Nama Rumah Sakit Yang Anda Pilih" value="<?= $res['hospitals_name'] ?>" readonly></div>
            <p>Tanggal Pertemuan Konsultasi*</p>
            <div class="form-group"><input class="form-control" type="date" name="dok" id="dok"></div>
            <?= form_error('dok', '<small class="text-danger pl-3">', '</small>'); ?>
            <p>Keluhan*</p>
            <div class="form-group"><textarea class="form-control" name="keluhan" id="keluhan" placeholder="Keluhan Medis Yang Anda Rasakan" rows="14"></textarea></div>
